        ...
        <DialogPanel class="w-full max-w-lg rounded-2xl bg-business-dark border border-business-light p-6 shadow-2xl max-h-[80vh] overflow-y-auto">
          ...
          <form @submit.prevent="savePromptTemplate" class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <p class="text-[9px] text-slate-600 mb-2">可用变量：{stock_snapshot} {capital_flow_snapshot} {sector_context} {market_context} {holding_context} {commentary_snapshot} {analysis_snapshot}（兼容旧变量：{stock_basic} {price_data} {money_flow} {holding} {market_sentiment} {mainline}）</p>
...
const defaultTemplateContent = `你是A股交易决策助手，熟悉震荡市和结构性牛市，风格偏短中线执行。

请只基于以下资料输出聚焦交易决策的 Markdown，总字数控制在 320-520 字。
