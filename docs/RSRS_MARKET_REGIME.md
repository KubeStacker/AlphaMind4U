...
#### 步骤4：检测连阳和上升趋势（v2.0新增）

为了更准确地识别市场状态，特别是连阳市场，新增了趋势检测：
...
| RSRS_Zscore | 趋势检测 | 最终状态 | 策略 |
|------------|---------|---------|------|
| > 0.7 | - | **Attack (进攻)** | 牛市，动量跟踪，放宽风控 |
| < -0.7 | 上升趋势 | **Attack (进攻)** | 连阳市场，覆盖RSRS判断 |
| < -0.7 | 连阳 | **Attack/Balance** | 根据强度调整 |
| < -0.7 | 无趋势 | **Defense (防守)** | 熊市，均值回归，严控回撤 |
| -0.7 ~ 0.7 | 上升趋势 | **Attack (进攻)** | 提升为进攻模式 |
