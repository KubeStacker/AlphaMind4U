...
- **主要服务**:
  ...
  - `alpha_model_t10.py`: T10结构狙击者模型算法
...
| strategy_version | VARCHAR(30) | 策略版本（T10_Structural_Sniper/T8_Anti_Chase） | DEFAULT 'T10_Structural_Sniper' |
...
│   ├── alpha_model_t10.py              # T10模型算法（结构狙击者）
...
#### 4.3.1 AlphaModelT10StructuralSniper (T10模型)

**文件**: `backend/services/alpha_model_t10.py`
...
- `run_full_pipeline(trade_date, params, top_n)`: 执行完整推荐流程
  ...
  - 流程: Layer1(板块) -> Layer2(基因) -> Layer3(狙击) -> Layer4(评分)
...
T10模型是一个基于纯L1数据（价量）驱动的结构狙击模型，其漏斗架构如下：
1. **战场选择**: 只在MA20向上的板块中寻找个股，并进行基本面排雷。
2. **股性基因**: 筛选近20日有涨停或大阳线的活跃股，拒绝死股。
3. **狙击形态**: 识别"极致缩量"和"黄金坑"回调形态。
4. **筹码评分**: 综合缩量极致分、换手健康分和RPS护盘分。
...
输入: trade_date, params, top_n
  ...
  │     └─> 基本面/市值过滤
  │
  ├─> Layer 2: 股性基因 (Active Gene)
  │     ├─> 涨停/大阳基因检测 (20日内)
  │     └─> 流动性门槛 (成交额 > 1亿)
  │
  ├─> Layer 3: 狙击形态 (Sniper Setup)
  │     ├─> 极致缩量识别 (Vol Ratio < 0.6)
  │     ├─> 黄金坑形态 (-3% ~ 1%)
  │     └─> MA20支撑验证
  │
  ├─> Layer 4: 筹码评分 (Scoring)
  │     ├─> 缩量极致分 (40%)
  │     ├─> 换手健康分 (30%)
  │     └─> RPS护盘分 (30%)
  │
  └─> 输出: recommendations (按total_score和阴线低吸排序)
```
...
**2. 极致缩量分 (F1)**:
```
f1_score = 40 (if vol_ratio < 0.55) else 30 (if vol_ratio < 0.6)
```

**3. 最终评分**:
```
total_score = (f1_vol_score) + (f2_turnover_score) + (f3_rps_score) + negative_bonus
```
